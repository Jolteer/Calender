...
class Calendar {
  ...
  updateCurrentPeriod() {
    if (this.currentView === "month") {
      ...
      this.elements.currentPeriod.textContent = `${this.formatDateShort(
        weekStart
      )} - ${this.formatDateShort(weekEnd)}`;
    }
  }
    ...
    for (let i = 0; i < rows; i++) {
      ...
      for (let j = 0; j < 7; j++) {
        ...
        if (cellIndex < firstDayOfWeek) {
          // Previous month days
          html += this.createMonthCell(
            "prev",
            prevLastDate - firstDayOfWeek + cellIndex + 1,
            year,
            month
          );
  ...
  renderWeeklyView() {
    ...
    this.elements.weeklyCalendarBody.innerHTML = this.generateWeeklyGrid(
      weekStart,
      today
    );
  ...
  renderEventsInMonthlyView() {
    ...
    Object.entries(eventsByDate).forEach(([date, events]) => {
      const cell = document.querySelector(
        `.monthly-calendar td[data-date="${date}"]`
      );
      if (!cell) return;
  ...
  renderEventsInWeeklyView() {
    ...
    Object.entries(eventsByDate).forEach(([date, events]) => {
      const cell = document.querySelector(
        `.weekly-day-cell[data-date="${date}"]`
      );
      if (!cell) return;
  ...
  openEventModal(date, hour = null) {
    ...
    if (hour !== null) {
      this.elements.eventStartTime.value = `${String(hour).padStart(
        2,
        "0"
      )}:00`;
      this.elements.eventEndTime.value = `${String(hour + 1).padStart(
        2,
        "0"
      )}:00`;
    }
